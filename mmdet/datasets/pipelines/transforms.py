...
class MixUp(object):
    def __init__(self, p=0.3, lambd=0.5):
        ...
        self.labels2= None
        self.masks2 = None
        self.semantic_seg2 = None

    def __call__(self, results):
        img1, boxes1, labels1, masks1, semantic_seg1 = [
            results[k] for k in ('img', 'gt_bboxes', 'gt_labels','gt_masks','gt_semantic_seg')
        ]
        if random.random() < self.p and self.img2 is not None and img1.shape[1]==self.img2.shape[1]:
            ...
            mixup_masks = np.hstack((masks1, self.masks2)) 
            mixup_semantic_seg = np.zeros([height, width, 1], dtype='float32')
            mixup_semantic_seg[:semantic_seg1.shape[0], :semantic_seg1.shape[1], :] = semantic_seg1.astype('float32') * self.lambd
            mixup_semantic_seg[:self.semantic_seg2.shape[0], :self.semantic_seg2.shape[1], :] += self.semantic_seg2.astype('float32') * (1. - self.lambd)
            mixup_semantic_seg = mixup_semantic_seg.astype('unit8')           
            results['img'] = mixup_image
            results['gt_bboxes'] = mixup_boxes
            results['gt_labels'] = mixup_label
            results['gt_masks'] = mixup_masks
            results['gt_semantic_seg'] = mixup_semantic_seg
        else: 
            pass
        self.img2 = img1
        self.boxes2 = boxes1
        ...
        self.masks2 = masks1
        self.semantic_seg2 = semantic_seg1
        return results
